				...
				<div class="form-group">
					<div class="col-sm-10">
						<textarea class="form-control" name="postcontent" id="postcontent" rows="10"
							...
							placeholder="${post.postcontent}" > ${post.postcontent}</textarea>
					...
					<c:forEach items="${fileVo}" var="fileVo">
						<div class="col-sm-10">
							<ul class="fileTr">
								<li>
									<a class="file" id="DownGOGO" href="">${fileVo.filename}</a>
									<a id="filedeleteGOGO" onclick="return confirm('정말로 삭제하시겠습니까?')" href="${pageContext.request.contextPath}/fileDelete?fileid=${fileVo.fileid}&postid=${post.postid}" class="btn btn-default pull-right">X</a>
								</li>
							</ul>
						</div>
					</c:forEach>
